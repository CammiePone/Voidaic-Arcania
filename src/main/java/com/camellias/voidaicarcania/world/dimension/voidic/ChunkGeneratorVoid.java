...
public class ChunkGeneratorVoid implements IChunkGenerator
{
    ...
    protected static final IBlockState VOID_ROCK = ModBlocks.BLOCK_VOIDSTONE.getDefaultState();
    ...
    public ChunkGeneratorVoid(World p_i47241_1_, boolean p_i47241_2_, long p_i47241_3_, BlockPos p_i47241_5_)
    {
    ...
    public void buildSurfaces(ChunkPrimer primer)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            for (int j = 0; j < 16; ++j)
            {
                int k = 1;
                int l = -1;
                IBlockState iblockstate = VOID_ROCK;
                IBlockState iblockstate1 = VOID_ROCK;
                ...
                for (int i1 = 127; i1 >= 0; --i1)
                {
                    ...
                    else if (iblockstate2.getBlock() == ModBlocks.BLOCK_VOIDSTONE)
                    {
                        if (l == -1)
                        {
                            l = 1;
...
/*public class ChunkGeneratorVoid implements IChunkGenerator
{
	protected static final IBlockState MAIN_BLOCK = Blocks.AIR.getDefaultState();
    protected static final IBlockState AIR = Blocks.AIR.getDefaultState();
    ...
    public ChunkGeneratorVoid(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
    {
        this.world = world;
        ...
        for (int i = 0; i < 16; ++i)
        {
            for (int j = 0; j < 16; ++j)
            {
                ...
                for (int i1 = 127; i1 >= 0; --i1)
                {
                    ...
                    else if (iblockstate2.getBlock() == Blocks.AIR)
                    {
                        if (l == -1)
                        {
                            l = 1;
    ...
    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
    {
        ...
        for (int i = -12; i <= 12; ++i)
        {
            for (int j = -12; j <= 12; ++j)
            {
                ...
                if (k * k + l * l > 4096L && ChunkGeneratorVoid.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
                {
                    float f3 = (MathHelper.abs((float)k) * 3439.0F + MathHelper.abs((float)l) * 147.0F) % 13.0F + 9.0F;
                    f = (float)(p_185960_3_ - i * 2);
                    f1 = (float)(p_185960_4_ - j * 2);
