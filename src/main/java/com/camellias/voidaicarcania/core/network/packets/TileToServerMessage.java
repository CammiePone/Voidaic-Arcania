...
public class TileToServerMessage implements IMessage, IMessageHandler<TileToServerMessage, IMessage>
{
	private long pos;
	private NBTTagCompound nbt;
	
	public TileToServerMessage() {}
	
	public TileToServerMessage(BlockPos pos, NBTTagCompound nbt)
	{
		this.pos = pos.toLong();
		this.nbt = nbt;
	}
	...
	public IMessage onMessage(final TileToServerMessage message, final MessageContext ctx)
	{
		...
		mainThread.addScheduledTask(new Runnable()
		{
			public void run()
			{
				...
				if((world != null) && (bp != null))
				{
					...
					{
						((TEGeneric)te).messageFromClient(message.nbt == null ? new NBTTagCompound() : message.nbt, ctx.getServerHandler().player);
					}
				}
			}
		});
